...
import { addResolverMetadata, getClassName, getClassOrUndefined } from './resolvers.utils';

let TypeGqlResolver: Function;
let getMetadataStorage: Function;
try {
  TypeGqlResolver = require('type-graphql').Resolver;
  getMetadataStorage = require('type-graphql/dist/metadata/getMetadataStorage').getMetadataStorage;
} catch (e) {}

export function Resolver();
export function Resolver(name: string);
export function Resolver(classType: Type<any>, options?: ResolverClassOptions);
export function Resolver(typeFunc: ClassTypeResolver, options?: ResolverClassOptions);
export function Resolver(
  ...
  return (target: Object | Function, key?: string | symbol, descriptor?: any) => {
    ...
    if (!isString(nameOrType)) {
      TypeGqlResolver &&
        lazyMetadataStorage.store(() => TypeGqlResolver(nameOrType, options)(target as Function));
    }
  };
}
