...
export function Mutation(typeFunc: ReturnTypeFunc, options?: AdvancedOptions): MethodDecorator;
export function Mutation(
  nameOrType?: string | ReturnTypeFunc,
  options?: AdvancedOptions,
): MethodDecorator {
  return (target: Object | Function, key?: string | symbol, descriptor?: any) => {
    const name = isString(nameOrType) ? nameOrType : (options && options.name) || undefined;

    addResolverMetadata(Resolvers.MUTATION, name, target, key, descriptor);
    if (nameOrType && !isString(nameOrType)) {
      TypeGqlMutation &&
        lazyMetadataStorage.store(() =>
          TypeGqlMutation(nameOrType, options)(target as Function, key, descriptor),
        );
    }
  };
}
